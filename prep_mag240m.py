...
def preprocess(data_path, host, host_size, p2p_group, p2p_size):
    ...
    partitioned_feat_dir = osp.join(data_path, f"mag240m_kddcup2021/partitioned_feat")
    # if not osp.exists(partitioned_feat_dir):
    #     os.mkdir(partitioned_feat_dir)
    ...
    if not osp.exists(osp.join(partitioned_feat_dir, f'{host_size}h')):
        os.makedirs(osp.join(partitioned_feat_dir, f'{host_size}h'))
    
    torch.save(global2host.cpu(),
               osp.join(partitioned_feat_dir, f'{host_size}h/global2host.pt'))
    t2 = time.time()
    print(f'g2h {t2 - t1}')
    ...
    for host in range(host_size):
        ...
        torch.save(replicate.cpu(),
                   osp.join(partitioned_feat_dir, f'{host_size}h/replicate{host}.pt'))
        t3 = time.time()
        ...
        torch.save(local_orders.cpu(),
                   osp.join(partitioned_feat_dir, f'{host_size}h/local_order{host}.pt'))
        t4 = time.time()
        print(f'order {t4 - t3}')
...
preprocess('/data/juelin/dataset/OGBN/', 0, 1, 1, 4)
